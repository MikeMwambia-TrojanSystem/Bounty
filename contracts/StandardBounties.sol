...
contract StandardBounties is Pausable {
  ...
  uint public totalFulfilments;
  string public latestIpfsHash;
  string public latestFulfilmentHash;
  ...
  modifier onlyFulfiller(uint _bountyId, uint _fulfillmentId) {
    ...
    modifier issuerOrFulfiller(uint _bountyId, uint _fulfillmentId) {
        require(msg.sender == bounties[_bountyId].issuer ||
      ...
      returns (uint)
  {
    ...
    /*
     * @dev setLatestIpfsHash() sets the latest ipfsHash of the bounties
     * @param _ipfsHash
     * @param _bountyId
     */
    ...
    /*
     * @dev pauseBounty in case of an emergency, only by owner
     */
    function pauseBounty() public {
        pause();
    }

    /*
     * @dev uppauseBounty if all issues have been resolved, only by owner
     */
    function unpauseBounty() public {
        unpause();
    }

    /*
     * @dev withdrawFundsOnEmergency in case of an emergency, only by owner
     * @param send all funds to emergency address
     */
    function withdrawFundsOnEmergency(address emergencyAddress)
    public
    whenPaused
    {
        uint balance = address(this).balance;
        emergencyAddress.transfer(balance);
    }
